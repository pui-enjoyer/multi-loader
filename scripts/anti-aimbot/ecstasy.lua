-- downloaded from https://discord.gg/DQsVz6H3d4
...
local function print_raw(r, g, b, ...)
    client.color_log(r, g, b, 'ecstasy\0')
...
local qhouss_config_system do
    ...
    function qhouss_config_system.export_to_str()
        ...
        return base64.encode(json.stringify(config_result)) .. '_ecstasy'
    end

    function qhouss_config_system.import_from_str(str)
        str = str:gsub('_ecstasy', '')
...
do
    ...
    do
        gui.enabled = menu.new_item(ui.new_checkbox, "AA", "Anti-aimbot angles", string.format('\a%secstasy', utils.to_hex(182, 182, 101, 255)))
        : record("gui", "enabled")
...
do
    ...
    local function modify_jitter()
        ...
        if ctx.yaw_jitter == "ecstasy" then
            ...
            local ecstasy_mode  = ctx.jitter_mode
            local ecstasy_cycle = ctx.ecstasy_cycle
            local ecstasy_delay = ctx.ecstasy_delay

            local ways = ecstasyyaw_ways[ecstasy_mode]
            ...
            if ecstasy_cycle ~= 4 and not delay_data.is_delay and delay_data.ticks % ecstasy_cycle == 0 then
                delay_data.is_delay = true
            end

            local ignore_yaw = false
            if delay_data.is_delay then
                if delay_data.current < ecstasy_delay then
                    delay_data.current = delay_data.current + 1

                    if ctx.ecstasy_safe then
                        local lp = entity.get_local_player()
                ...
                local angle = 0
                if ecstasy_mode == "2-Way" and (ctx.body_yaw == "Jitter" or ctx.body_yaw == 'Randomize Jitter') then
...
do
    ...
    widgets.items = menu.new_item(ui.new_multiselect, "AA", "Anti-aimbot angles", merge { "- Items", "\n", "widgets::items" }, { "Watermark", "Keybinds", "Velocity Warning", "", "Damage Indicator", "On-Screen Logs", 'Hit Rate' })
...
LPH_NO_VIRTUALIZE(function ()
    ...
    do
        ...
        client.set_event_callback('paint', function ()
            ...
            local str = decorations.wave('ecstasy', globals.realtime(), r, g, b, 200, 255, 255, 255, 200)
...
do
    local function set_custom_list(ctx, list)
        ...
        ctx.ecstasy_cycle = list.ecstasy_cycle:get()
        ctx.ecstasy_delay = list.ecstasy_delay:get()
        ctx.ecstasy_safe = list.ecstasy_safe:get()
    ...
    angles.custom = { } do
        ...
        for i = 0, #e_statement do
            ...
            list.ecstasy_cycle = menu.new_item(ui.new_slider, "AA", "Anti-aimbot angles", merge { "Delay Cycle", "\n", "custom_", "delay_cycle_", state }, 5, 200, 50, true, '', 1, { [5] = "Off" })
            : record("aa", merge { "custom", "::", state, "::", "ecstasy_cycle" })
            : save()

            list.ecstasy_delay = menu.new_item(ui.new_slider, "AA", "Anti-aimbot angles", merge { "Delay Time", "\n", "custom_", "ecstasy_delay", state }, 5, 30, 15)
            : record("aa", merge { "custom", "::", state, "::", "ecstasy_delay" })
            : save()

            list.ecstasy_safe = menu.new_item(ui.new_checkbox, "AA", "Anti-aimbot angles", merge { "Safe Yaw", "\n", "custom_", "safe_yaw_", state })
            : record("aa", merge { "custom", "::", state, "::", "ecstasy_safe" })
            : save()
            ...
            list.ecstasy_safe:set_callback(reset_delay)
            list.ecstasy_delay:set_callback(reset_delay)
            list.ecstasy_cycle:set_callback(reset_delay)
    ...
    angles.recommended = {
        ...
        ['Slow Walk'] = function (ctx)
            ...
            ctx.yaw = '180'
            ctx.yaw_offset = 10
            ctx.yaw_jitter = 'ecstasy'
            ctx.jitter_mode = '2-Way'
            ctx.jitter_offset = 60
            ctx.jitter_randomization = 15
            ctx.ecstasy_cycle = 32
            ctx.ecstasy_delay = 18
            ctx.ecstasy_safe = true
            ctx.body_yaw = 'Jitter'
            ctx.body_yaw_offset = -40
        end,
...
do
    ...
    shared.data = { }
    shared.icon_data = { }
    shared.link = "wss://exscord.dev/ecstasy_ws"
    ...
    shared.attach = function (condition)
        ...
        for i = 1, globals.maxplayers() do
            ...
            if logo_id then
                scoreboard.set_icon(i, string.format("https://exscord.dev/ecstasy/icons/%s.png", logo_id))
        ...
        if angles.type:get() == "Custom" then
            ...
            if yaw_value ~= "Off" then
                ...
                if list.yaw_jitter:get() ~= "Off" then
                    ...
                    if list.yaw_jitter:get() == "ecstasy" then
                        list.jitter_mode:display()
                        list.ecstasy_safe:display()
                        list.ecstasy_cycle:display()
                        list.ecstasy_delay:display()

                        ui.set_enabled(list._delay.ref, list.ecstasy_cycle:get() ~= 5)
                    end
                end
            end
