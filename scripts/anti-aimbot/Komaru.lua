...
local L70 = { reset_once = false, hitgroup_names = { [0] = "body", "head", "chest", "stomach", "left arm", "right arm", "left leg", "right leg", "neck", "?", "gear" }, fire_total_hits = 0, post_total_hits = 0, current_condition = "", mode = "back", is_defensive_running = false, banana = false, old_tick_count = 0, yaw_increment_spin = 0, tickbase_max, tickbase_diff, current_cmd, bomb_defused = false, bomb_exploded = false, pulse = 240, started = 10, smooth_KOMARU = 0, smooth_dt = 0, smooth_os = 0, smooth_pc = 0, smooth_bo = 0, current_desync = 0, fake_fakelag = 0, cur = 0, is_defusing = false, desync_rect_dist = 0, dt_os_text_anim = 0, current_cond_text_anim = 0, smooth_KOMARU_code = 0, smooth_dt_2 = 0, smooth_stance = 0, dt_vertical_dist = 0, jumping = false, on_ground = false, rage_fired = false, last_jump_ducked = false, landing = false, waiting_scan_text = 0, hittable = false, defensive_risk = 0, smooth_defensive_bar = 0, smooth_left_arrow = 0, smooth_right_arrow = 0, smooth_up_arrow = 0, smooth_arrow_alpha = 0 }
...
local L75 = { ["user"] = "by 7days", ["build"] = "code" }
local L76 = L24.new_checkbox("AA", "Anti-aimbot angles", "KOMARU - " .. L22.lower(L75["user"]))
local L77 = nil;
local L78 = nil;
local L79 = nil;
local L80 = nil;
...
function watermark(L165, L166)
    ...
    local L167 = ""
    local L168 = "KOMARU [" .. L22.upper(L75["build"]) .. "]" .. " | " .. L22.upper(L75["user"]) .. " | " .. L23.floor(L28.latency() * 1000) .. "MS"
    if L166 == "small" then
        L167 = "-"
        L168 = "KOMARU   [" .. L22.upper(L75["build"]) .. "]" .. "   |   " .. L22.upper(L75["user"]) .. "   |   " .. L23.floor(L28.latency() * 1000) .. "MS"
...
local L197 = function(L192, L193, L194, L195, L196)
    ...
    elseif not L132(L24.get(L90["visuals"][6]), "animations on scope") and L24.get(L90["visuals"][0]) == "code alpha" then
...
local L401 = function(L359, L360, L361, L362, L363)
    ...
    if L24.get(L90["visuals"][0]) ~= "code alpha" then
        L70.smooth_KOMARU_code = 0;
        L70.smooth_dt_2 = 0;
        L70.smooth_stance = 0;
    ...
    if L24.get(L360) == "minimal (og)" then
        ...
        L70.smooth_KOMARU = L145(L361) and L136(L70.smooth_KOMARU, -2, L26.frametime() * 15) or L136(L70.smooth_KOMARU, L25.measure_text(L371, L376) / 2, L26.frametime() * 15)
        table.insert(L364, { text = L376, r = 210, g = 210, b = 210, a = 255, size = L23.ceil(L70.smooth_KOMARU) })
        ...
        elseif L24.get(L360) == "code alpha" then
            local L389 = ("KOMARU\a%s code"):format(L22.format("%02X%02X%02X%02X", L365, L366, L367, L23.max(L368, 100)))
            local L390 = "DT"
            local L391 = "ON SHOT"
            ...
            KOMARU_code_text_size = { L25.measure_text(L371, L389) }
            L70.smooth_KOMARU_code = L145(L361) and L136(L70.smooth_KOMARU_code, -2, L26.frametime() * 15) or L136(L70.smooth_KOMARU_code, KOMARU_code_text_size[1] / 2, L26.frametime() * 15)
            table.insert(L364, { text = L389, r = 240, g = 240, b = 240, a = 255, size = L23.floor(L70.smooth_KOMARU_code), txt_measure = { L25.measure_text(L371, L389) } })
...
g_notify = L415:new()
local L440 = { 'جدا الحمد لله أبي', "Buy komaru.lua discord : vedro6046", "KOMARU DA BEST", "Oops. tapped with KOMARU.LUA", "Get good. Get KOMARU.LUA", "skeet invite code in morse: ... .-- ..-. -.-- -... .-- ..-. -... .--- --.. -... .-.. -.- .... ..-. .-.. -.- --. .. .-. .--. --. .-.. --.- --.- - -.-- .---- -..- . .-- -.- -.-- --.- ---.. .-.. .... ... ...- --.. -..- -.. .--. -..- -- -... - -.--", 'Tapped and jelous? "Cheap and good? Only KOMARU.LUA"', "im llegal for having KOMARU.LUA", " Embarassing. get KOMARU.LUA ", "Stop being shit, and get KOMARU.lua", "Easy for KOMARU.lua", "Too easy. Get komaru.lua" }
last_random = 0;
new_random = 0;
textalhao = ""
say_time = 0;
...
local L479 = function(L473)
    ...
    if L132(L24.get(L90["misc"][1]), "blinded") then
...
local L610 = function(L601, L602, L603, L604)
    ...
    if L24.get(L90["anti-aim"][0]) == "gamesense" then
        ...
        if L24.get(L90["anti-aim"][0]) == "KOMARU (dont use)" then
            L148()
        end
    end
end;
...
local L734 = function()
    ...
    if not L715 then
        ...
        L70.smooth_KOMARU = -1080;
        L70.smooth_dt = -1080;
        L70.smooth_os = -1080;
        L70.smooth_pc = -1080;
        L70.smooth_bo = -1080;
        L70.smooth_KOMARU_code = -1080;
    ...
    for L728, L729 in L9({ L82, L83 }) do
        ...
        for L730, L731 in L10(L729) do
            if L718 == "gamesense" and L729 == L82 then
                L80 = L729
            elseif L718 == "KOMARU (dont use)" and L729 == L83 then
                L79 = L729
            end;
            for L732, L733 in L10(L731) do
                L716 = (L718 == "gamesense" and L729 == L82 or L718 == "KOMARU (dont use)" and L729 == L83) and L84[L730] == L719 and L717 == "anti-aim" and L715 and (L733 == L729[L730][0] or L24.get(L729[L730][0]))
                if L718 == "gamesense" and L729 == L82 then
...
local L765 = function()
    ...
    if L132(L24.get(L90["misc"][2]), "config changes") then
        g_notify:add("imported KOMARU anti-aim", nil, 5)
    end
end;
local L773 = function()
    local L766 = ""
    ...
    if L132(L24.get(L90["misc"][2]), "config changes") then
        g_notify:add("exported KOMARU anti-aim", nil, 5)
    end
end;
