...
local client_set_event_callback, client_unset_event_callback, client_userid_to_entindex, entity_get_local_player, ui_get, ui_set, ui_new_checkbox, ui_new_combobox, ui_set_callback, ui_set_visible = client.set_event_callback, client.unset_event_callback, client.userid_to_entindex, entity.get_local_player, ui.get, ui.set, ui.new_checkbox, ui.new_combobox, ui.set_callback, ui.set_visible
...
--------------------------------------------------------------------------------
local enable_ref
local head_enable_ref
local head_sound_ref
local body_enable_ref
local body_sound_ref
local kill_enable_ref
local kill_sound_ref
local volume_ref
...
local function on_player_hurt(e)
	if client_userid_to_entindex(e.attacker) == entity_get_local_player() then
		local is_head = e.hitgroup == 1
		if is_head and not ui_get(head_enable_ref) then return end
		if not is_head and not ui_get(body_enable_ref) then return end
		local sound_file = sound_name_to_file[is_head and ui_get(head_sound_ref) or ui_get(body_sound_ref)]
		if sound_file then
...
local function on_player_blind(e)
	if client_userid_to_entindex(e.attacker) == entity_get_local_player() then
		if not ui_get(body_enable_ref) then return end
...
local function on_player_death(e)
	...
	if victim ~= nil and victim ~= 0 and client_userid_to_entindex(e.attacker) == me and victim ~= me and entity.is_enemy(victim) and ui_get(kill_enable_ref) then
		local sound_file = sound_name_to_file[ui_get(kill_sound_ref)]
		if sound_file then
			for i=1, ui_get(volume_ref) do
				sndplaydelay:invoke_callback(0, sound_file)
			end
...
local function on_hit_sound_toggle(ref, value)
	local state = enable_ref:get()
	local head = state and ui_get(head_enable_ref)
	local body = state and ui_get(body_enable_ref)
	local kill = state and ui_get(kill_enable_ref)
	ui_set_visible(head_enable_ref, state)
	ui_set_visible(head_sound_ref, head)
	ui_set_visible(body_enable_ref, state)
	ui_set_visible(body_sound_ref, body)
	ui_set_visible(kill_enable_ref, state)
	ui_set_visible(kill_sound_ref, kill)
	ui_set_visible(volume_ref, head or body or kill)
end
...
local function init()
    ...
    ui.new_label('VISUALS', 'Colored models', '=------------  [ Custom Hitsound ]  -----------=')
	enable_ref	  = uix.new_checkbox("VISUALS", "Colored Models", "Custom Hitsound")
	head_enable_ref = ui_new_checkbox("VISUALS", "Colored Models", "Enable Head Sound")
	head_sound_ref  = ui_new_combobox("VISUALS", "Colored Models", "Head Shot Sound", sound_names)
	body_enable_ref = ui_new_checkbox("VISUALS", "Colored Models", "Enable Body Sound")
	body_sound_ref  = ui_new_combobox("VISUALS", "Colored Models", "Body Shot Sound", sound_names)
	kill_enable_ref = ui_new_checkbox("VISUALS", "Colored Models", "Enable Kill Sound")
	kill_sound_ref  = ui_new_combobox("VISUALS", "Colored Models", "Kill Sound", sound_names)
	volume_ref	  = ui.new_slider("VISUALS", "Colored Models", "Sound Volume", 1, 100, 1, true, "%")
	ui_set(head_enable_ref, true)
	ui_set(body_enable_ref, true)
	if sound_name_to_file["rust"] then
		ui_set(kill_sound_ref, "rust")
	end

	enable_ref:on("change", on_hit_sound_toggle)
	ui_set_callback(head_enable_ref, on_hit_sound_toggle)
	ui_set_callback(body_enable_ref, on_hit_sound_toggle)
	ui_set_callback(kill_enable_ref, on_hit_sound_toggle)
	...
	on_hit_sound_toggle(enable_ref)
end
