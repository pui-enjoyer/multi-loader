...
menu.general = {
	config = {
		...
		dashboard_auth = groups.angles:button("\vGet Auth Code", NILFN),
		menu.space(groups.angles),
		menu.header(groups.angles, "\a9ACD32FF\aFFFFFFFF Discord auth"),
		discord_auth_hint = groups.angles:label("\f<p>Get a code, then in Discord use slash: /auth"),
		discord_link = groups.angles:button("\vGet Discord Code", NILFN),
		hide_amnesia_username = groups.other:checkbox("Hide Amnesia Username", false),
		menu.space(groups.other),
		menu.header(groups.other, "\a9ACD32FF\aFFFFFFFF Socials"),
		discord_server = groups.other:button("\vDiscord Server", function()
			local ok = pcall(function()
				panorama.open().SteamOverlayAPI.OpenExternalBrowserURL("https://dsc.gg/gamesensical")
			end)
			if not ok then
				clipboard.set("https://dsc.gg/gamesensical")
				amnesia.print("\aB6B665Copied Discord link to clipboard: \vhttps://dsc.gg/gamesensical")
			end
		end),
		cloud_dashboard = groups.other:button("\vCloud User Dashboard", function()
			local ok = pcall(function()
				panorama.open().SteamOverlayAPI.OpenExternalBrowserURL("https://amnesia.cfd/#dashboard")
			end)
			if not ok then
				clipboard.set("https://amnesia.cfd/#dashboard")
				amnesia.print("\aB6B665Copied Dashboard link to clipboard: \vhttps://amnesia.cfd/#dashboard")
			end
		end),
		menu.space(groups.other),
	},
...
vars.misc = {
	...
	buybot = menu.feature(groups.angles:checkbox(amnesia.t("Buy Bot")), function (parent)
		return {
			primary = groups.angles:combobox("\f<p>" .. amnesia.t("Primary"), {"-", "AWP", "SCAR20/G3SG1", "Scout", "M4/AK47", "Famas/Galil", "Aug/SG553", "M249/Negev", "Mag7/SawedOff", "Nova", "XM1014", "MP9/Mac10", "UMP45", "PPBizon", "MP7"}),
			secondary = groups.angles:combobox("\f<p>" .. amnesia.t("Secondary"), {"-", "CZ75/Tec9/FiveSeven", "P250", "Deagle/Revolver", "Dualies"}),
			grenades = groups.angles:multiselect("\f<p>" .. amnesia.t("Grenades"), {"HE Grenade", "Molotov", "Smoke", "Flash", "Decoy"}),
			utilities = groups.angles:multiselect("\f<p>" .. amnesia.t("Utilities"), {"Armor", "Helmet", "Zeus", "Defuser"}),
		}, true
	end),
}
...
do -- states
	...
	for i, v in ipairs(antiaim.states) do
		...
		ctx.ds_sens	= new({id, "snap", "sensitivity"}, groups.other:slider("\f<p>" .. amnesia.t("Sensitivity") .. "\f<_p>", 1, 100, 100, true, "%"))
		ctx.ds_ping	= new({id, "snap", "ping_target"}, groups.other:slider("\f<p>" .. amnesia.t("Ping value") .. "\f<_p>", 0, 200, 20, true, "ms"))
...
misc.clantag = {
	...
	run = a(function (self)
		...
		vars.misc.clantag:set_callback(function (this)
			if this.value then
				-- Включаем amnesia клантег - отключаем гейм сенс клантег
				self.enabled = true
				callbacks.net_update_end:set(self.work)
				refs.misc.clantag:set_enabled(false)
				refs.misc.clantag:override(false)
			else
				-- Выключаем amnesia клантег - восстанавливаем оригинальное состояние
				self.enabled = false
		...
		defer(function ()
			-- Не трогаем клантег при инициализации, если amnesia клантег выключен
...
do
	...
	if type(s.online) == "number" then
		amnesia.online_count = math.max(0, math.floor(s.online))
	else
	...
	amnesia.expiry = s.expiry or "lifetime"
	...
	pcall(function()
		...
		client.set_event_callback("round_start", function()
			...
			http.get(url, function(a, b)
				...
				if amnesia.update_scoreboard then
					amnesia.update_scoreboard()
				end
...
do
	...
	amnesia.update_scoreboard = update_scoreboard
...
do
	...
	local function update_fake_online()
		...
		if not fake_online.initialized then
			...
			amnesia.online_count = fake_online.current
			if menu.info and menu.info.online then
